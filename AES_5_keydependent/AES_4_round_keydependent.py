...
class AES(object):
    ...
    def __declareVariables2(self):
        # declare the round variables input into the distinguishers
    ...
    def __gen_init_constrs(self, constants, tail):
        ...
        for i in [x for x in range(8, 32)] + [x for x in range(40, 64)] + [x for x in range(72, 96)] + [x for x in range(104, 128)]:
...
def main():
    inVec = [x for x in range(0, 8)] + [ x for x in range(32, 40)] + [ x for x in range(64, 72)]
    outVec = [ 0 ] 
#def __init__(self, matName, matrix, sboxName, sbox, sboxDim,dim, r, numberOfSbox, inVec, outVec):
    aes = AES('FIRST', first, 'AESMAT', matrix, 'AESSBOX', invSbox, 8, 128, 3, 16, inVec, outVec)
    print( '\n'.join( aes.getConstrs() ) )
